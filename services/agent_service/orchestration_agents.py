...
class QueryResolutionAgent:
    ...
    def run(self, message: InboundMessage, context: dict, intent: str | None = None) -> QueryResolution:
        decision = self._resolve_query_level(message, context, intent)
        resolution_context = {**context, "resolution_decision": decision}
        resolution = self._resolve(message, resolution_context, intent)
        return self._attach_resolution_decision(resolution, decision)
    ...
    def _resolve_query_level(
        self,
        message: InboundMessage,
        context: dict,
        intent: str | None,
    ) -> dict:
        """Compute the L1/L2/L3 decision once before response generation."""
        try:
            if self.resolution_engine is not None:
                return self.resolution_engine.resolve_query_level(
                    message.text, intent or "unknown", context.get("sentiment", "neutral"),
                )
            from services.resolution_service import resolve_query_level
            return resolve_query_level(
                message.text, intent or "unknown", context.get("sentiment", "neutral"),
            )
    ...
    @staticmethod
    def _attach_resolution_decision(resolution: QueryResolution, decision: dict) -> QueryResolution:
        """Attach the same L1/L2/L3 decision used for response generation to routing output."""
