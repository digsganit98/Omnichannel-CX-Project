...
class GroqGenerator:
    ...
    def generate_answer(self, query: str, contexts: list[dict], conversation_context: dict | None = None) -> dict:
        ...
        resolution_decision = ctx.get("resolution_decision") or {}
        resolution_level = str(resolution_decision.get("resolution_level", "")).upper()
        ...
        no_data_note = (
            ...
            if not graph_ctx_text and resolution_level != "L1" else ""
        )
        user_prompt = (
            f"{channel_rule}{lang_rule}\n\n"
            "Rules:\n"
            + _resolution_response_rule(resolution_level) +
            ...
            "- If context is insufficient, say only that you could not find enough verified information to answer completely. Do NOT claim escalation or ticket creation unless a ticket reference is present in the customer data.\n"
...
def _resolution_response_rule(resolution_level: str) -> str:
    if resolution_level == "L1":
        return (
            "- RESOLUTION LEVEL L1: This is an auto-resolvable knowledge-base query. "
            "Answer directly from the retrieved context. Do NOT use escalation wording, "
            "do NOT say a ticket has been or will be created, and do NOT say you cannot access "
            "the customer's account unless the customer explicitly asks for account-specific data.\n"
        )
    if resolution_level == "L2":
        return (
            "- RESOLUTION LEVEL L2: This may need customer-specific verification or assisted handling. "
            "Use assisted-resolution wording when needed, but do NOT say a ticket has been created "
            "unless a ticket reference is present in the customer data.\n"
        )
    if resolution_level == "L3":
        return (
            "- RESOLUTION LEVEL L3: This is a critical escalation case. Acknowledge the risk clearly "
            "and use urgent specialist-support wording. Do NOT invent a ticket reference.\n"
        )
    return ""
